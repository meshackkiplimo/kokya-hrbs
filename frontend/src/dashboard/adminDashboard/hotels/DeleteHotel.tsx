...
const DeleteHotel:React.FC<DeleteHotelProps> = ({hotelId}) => {
    const [deleteHotel] = hotelApi.useDeleteHotelMutation();
    const handleDelete = async () => {
        try {
            await deleteHotel(hotelId).unwrap();
            console.log('Hotel deleted successfully');
            // Optionally, you can add logic to refresh the hotel list or redirect
        } catch (error) {
  ...
  return (
      <div>
        ...
        <dialog id="delete_hotel_modal" className="modal modal-bottom sm:modal-middle z-[60]">
            <div className="modal-box bg-red-50 border border-red-200 w-11/12 max-w-md mx-auto my-8">
                <div className="flex items-center mb-4">
                    ...
                    <div>
                        <h3 className="font-bold text-lg text-red-800">Delete Hotel</h3>
                        <p className="text-sm text-red-600">This action cannot be undone</p>
                    </div>
                </div>
                
                <p className="text-gray-700 mb-6">
                    Are you sure you want to permanently delete this Hotel? All associated data will be removed.
                </p>
                ...
                <div className="modal-action flex flex-col sm:flex-row gap-3 mt-6">
                    ...
                    <button
                        ...
                        onClick={() => (document.getElementById('delete_hotel_modal') as HTMLDialogElement)?.close()}
                    >
                        Cancel
                    </button>
                </div>
        ...
        <button
            ...
            onClick={() => (document.getElementById('delete_hotel_modal') as HTMLDialogElement)?.showModal()}
        >
            <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
            </svg>
